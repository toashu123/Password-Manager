...
import { SignedIn, SignedOut, RedirectToSignIn, useUser, useSignIn } from "@clerk/clerk-react";
...
function App() {
  ...
  useEffect(() => {
    if (isLoaded && isSignedIn && user) {
      ...
      const pendingMFASetup = localStorage.getItem(`mfa_setup_pending_${user.id}`);
  ...
  useEffect(() => {
    if (isLoaded) {
      if (isSignedIn && user?.id) {
        const masterPassword = `SecureVault-${user.id}-${user.primaryEmailAddress?.emailAddress || 'user'}-2024`;
        ...
        try {
          ...
          toast.error("Failed to initialize encryption. Please refresh and try again.");
        }
      } else {
        try {
          clearMasterPassword();
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium text-sm sm:text-base">Loading SecureVault...</p>
          <p className="text-gray-400 text-xs sm:text-sm mt-2">Initializing secure session...</p>
        </div>
      </div>
    );
  }
      ...
      {isNewUser && !showMFASetup && (
        <div className="fixed bottom-3 sm:bottom-4 right-3 sm:right-4 left-3 sm:left-auto bg-blue-600 text-white p-3 sm:p-4 rounded-lg shadow-lg max-w-sm z-50 animate-slide-up">
          <div className="flex items-start gap-2 sm:gap-3">
            <span className="text-xl sm:text-2xl flex-shrink-0" role="img" aria-label="Security">🔐</span>
            <div className="flex-1 min-w-0">
              <h4 className="font-semibold mb-1 text-sm sm:text-base">Secure Your Account</h4>

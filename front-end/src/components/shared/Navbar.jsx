...
const Navbar = () => {
  ...
  // ✅ FIXED: Updated click outside handler
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (dropdownRef.current && !dropdownRef.current.contains(event.target)) {
        setShowDropdown(false);
      }
      // ✅ Only close mobile menu if clicking outside the entire mobile menu container
    ...
    <>
      <nav className="bg-slate-800 text-white shadow-lg relative z-40">
        <div className="flex justify-between items-center px-4 py-4">
          ...
          {/* ✅ FIXED: Wrapped hamburger and mobile menu in one ref container */}
          <div ref={mobileMenuRef} className="md:hidden">
            {/* Hamburger menu button */}
            <button
              className="text-white focus:outline-none hover:bg-slate-700 p-2 rounded transition-colors focus:ring-2 focus:ring-green-500"
              onClick={(e) => {
                e.stopPropagation();
            ...
            {/* ✅ Mobile Dropdown Menu - inside same ref */}
            {mobileMenuOpen && (
              <div className="absolute right-0 left-0 top-full mt-0 px-4 pb-4 pt-3 space-y-3 border-t border-slate-700 bg-slate-800 shadow-lg z-50">
                <button 
